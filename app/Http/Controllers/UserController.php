...
class UserController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($country == "0") {
            ...
            $users = User::where('role', 2)->orwhere('role', 3)->where('country', $country)->get();
        }
    ...
    public function store(UserRequest $request, User $model)
    {
        ...
        return redirect()->route('user.index', 'country=0')->withStatus(__('User successfully created.'));
    }
    /**
     ...
     * @return \Illuminate\View\View
     */
    public function edit(User $user, $type)
    {
        $id = $user->id;
        $arrArticles = '';
        if ($type == 3) {
            ...
            $arrArticles = Article::where('userId', $id)->where('offerType', $type)->orderByDesc('updated_at')->get();
        }
        $arrImages = [];
